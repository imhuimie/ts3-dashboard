...
export function ManagementSections({ sessionKey, selectedServerId, messages, unreadMessageCount, onMarkMessagesRead, onMessageSent, intent }: { sessionKey: string; selectedServerId: number; messages: MessageEntry[]; unreadMessageCount: number; onMarkMessagesRead: () => void; onMessageSent: (payload: { targetMode: number; target: number; message: string }) => void; intent: ManagementIntent | null; }) {
  ...
  useEffect(() => {
    ...
    setTokenForm((current) => ({
      ...
      tokenId1: scope === "channel-group" ? meta.channelGroups[0]?.id ?? 0 : meta.serverGroups[0]?.id ?? 0,
  ...
  return <div className="grid gap-4 xl:grid-cols-[1.1fr_0.9fr]">
    <form className="space-y-4 rounded-3xl border border-white/8 bg-white/[0.03] p-4" onSubmit={onSaveServer}>
      ...
      <div className="grid gap-3 md:grid-cols-2">
        ...
        <Input label="默认服务器组"><select className={inputClassName} value={serverAdminForm.defaultServerGroup} onChange={(event) => setNumberField("defaultServerGroup", Number(event.target.value))}>{serverGroups.map((group) => <option key={group.id} value={group.id}>{group.name}</option>)}</select></Input>
        <Input label="默认频道组"><select className={inputClassName} value={serverAdminForm.defaultChannelGroup} onChange={(event) => setNumberField("defaultChannelGroup", Number(event.target.value))}>{channelGroups.map((group) => <option key={group.id} value={group.id}>{group.name}</option>)}</select></Input>
        <Input label="默认频道管理员组"><select className={inputClassName} value={serverAdminForm.defaultChannelAdminGroup} onChange={(event) => setNumberField("defaultChannelAdminGroup", Number(event.target.value))}>{channelGroups.map((group) => <option key={group.id} value={group.id}>{group.name}</option>)}</select></Input>
      </div>
...
function ClientsView({ clients, dbClients, channels, serverGroups, onRefresh, onLoadDetail, onSaveEdit, onKick, onMove, onBan, onPoke, onDeleteDatabaseClients }: { clients: ClientSummary[]; dbClients: ClientDbEntry[]; channels: ChannelSummary[]; serverGroups: PermissionTarget[]; onRefresh: () => Promise<void> | void; onLoadDetail: (clientId: number) => Promise<ClientDetail>; onSaveEdit: (clientId: number, payload: { description: string; serverGroupIds: number[] }) => Promise<void>; onKick: (clientId: number, payload: { reason: string; mode: "server" | "channel" }) => Promise<void>; onMove: (clientId: number, payload: { targetChannelId: number; channelPassword?: string }) => Promise<void>; onBan: (clientId: number, payload: { reason: string; time: number }) => Promise<void>; onPoke: (clientId: number, payload: { message: string }) => Promise<void>; onDeleteDatabaseClients: (clientDbIds: number[]) => Promise<void>; }) {
  ...
  return <>
    ...
    <ModalFrame open={!!action} title={action ? `${action.mode === "edit" ? "编辑" : action.mode === "move" ? "移动" : action.mode === "kick" ? "踢出" : action.mode === "ban" ? "封禁" : "戳消息"} / ${action.client.nickname}` : "客户端操作"} onClose={() => !busy && setAction(null)}>
      {action?.mode === "edit" ? <div className="space-y-4"><Input label="描述"><textarea className={`${inputClassName} min-h-28`} value={editDescription} onChange={(event) => setEditDescription(event.target.value)} /></Input><Input label="服务器组"><select className={`${inputClassName} min-h-40`} multiple value={selectedGroupIds.map(String)} onChange={(event) => setSelectedGroupIds(Array.from(event.target.selectedOptions).map((item) => Number(item.value)))}>{serverGroups.map((group) => <option key={group.id} value={group.id}>{group.name}</option>)}</select></Input><div className="flex justify-end gap-2"><button className={smallSecondaryButtonClassName} type="button" onClick={() => setAction(null)} disabled={busy}>取消</button><button className={primaryButtonClassName} type="button" onClick={() => void submitAction()} disabled={busy || !detail}>保存</button></div></div> : null}
...
function GroupsView({ serverGroups, channelGroups, channels, clients, onRefreshServerGroups, onRefreshChannelGroups }: { serverGroups: PermissionTarget[]; channelGroups: PermissionTarget[]; channels: ChannelSummary[]; clients: ClientDbEntry[]; onRefreshServerGroups: () => Promise<void> | void; onRefreshChannelGroups: () => Promise<void> | void; }) {
  ...
  useEffect(() => {
    setBatchServerGroupId((current) => current || (serverGroups[0]?.id ?? 0));
  }, [serverGroups]);

  useEffect(() => {
    setBatchChannelGroupId((current) => current || (channelGroups[0]?.id ?? 0));
  }, [channelGroups]);
  ...
  async function openServerEditor(group: PermissionTarget) {
    ...
    setError(null);
    try {
      const members = await api.getServerGroupClients(group.id);
  ...
  async function openChannelEditor(group: PermissionTarget) {
    ...
    try {
      ...
      const members = channelId > 0 ? await api.getChannelGroupClients(group.id, channelId) : [];
      setChannelEditor({ id: group.id, name: group.name, type: group.type ?? 1, channelId, members: members.map((item) => item.cldbid) });
    } catch (cause) {
      ...
      <div className="grid gap-4 xl:grid-cols-2">
        <div className="rounded-3xl border border-white/8 bg-white/[0.03] p-4">
          ...
          <div className="mt-4 grid gap-3 xl:grid-cols-[220px_180px_1fr_auto]">
            <Input label="服务器组"><select className={inputClassName} value={batchServerGroupId} onChange={(event) => setBatchServerGroupId(Number(event.target.value))}>{serverGroups.map((group) => <option key={group.id} value={group.id}>{group.name}</option>)}</select></Input>
        ...
        <div className="rounded-3xl border border-white/8 bg-white/[0.03] p-4">
          ...
          <div className="mt-4 grid gap-3 xl:grid-cols-[220px_220px_180px_1fr_auto]">
            <Input label="频道组"><select className={inputClassName} value={batchChannelGroupId} onChange={(event) => setBatchChannelGroupId(Number(event.target.value))}>{channelGroups.map((group) => <option key={group.id} value={group.id}>{group.name}</option>)}</select></Input>
